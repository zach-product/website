...
export default class OnePost extends Component {
    ...
    render() {
        const { title, topics, published, content } = this.state
        return (
            <React.Fragment>
                <NavBar />
                <div className="container" style={stickyHeader}>
                    <div className="col-12 col-lg-10 offset-lg-1 pt-4">
                        ...
                        <hr className="mb-4" />
                        <p style={fontStyling}>{content.intro}</p>
                        {content.sections.map(section => {
                            return <React.Fragment>
                                    <h3 className="my-4">{section.header}</h3>
                                    <p style={fontStyling}>{section.body}</p>
                                    </React.Fragment>
                        })}
                        <p style={fontStyling}>{content.conclusion}</p>
                    </div>
                </div>
            </React.Fragment>
        )
